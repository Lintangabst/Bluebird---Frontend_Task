...
const Wishlist: React.FC = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      if (storedWishlist) {
        setLovedVehicles(JSON.parse(storedWishlist));
      }
    }, []);
      ...
      <div className="container mx-auto mt-8 pt-20 pb-20">
        ...
        <section className="mt-8">
          ...
          <div className="flex flex-wrap -mx-4">
            {filteredVehicles
              .filter(vehicleType => vehicleType.car_type.some(car => lovedVehicles.includes(car.vehicle)))
              .map(vehicleType => (
                <div key={vehicleType.id} className="flex w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 px-4 mb-8">
                  {vehicleType.car_type
                    .filter(car => lovedVehicles.includes(car.vehicle))
                    .map(car => (
                      <div key={car.vehicle} className="mb-8 flex-1 ml-8" onClick={() => setSelectedVehicle(car)}>
